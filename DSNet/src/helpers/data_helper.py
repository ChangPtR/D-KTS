...
class VideoDataset(object):
    def __init__(self, parent,keys: List[str]):
        self.keys = keys
        self.parent=parent
        self.datasets = self.get_datasets(parent,keys)
    ...
    def __getitem__(self, index):
        ...
        video_file = self.datasets[self.parent][key]

        seq = video_file['features'][...].astype(np.float32)
        gtscore = video_file['gt_score'][...].astype(np.float32)
    ...
    @staticmethod
    def get_datasets(parent,keys: List[str]) -> Dict[str, h5py.File]:
        # dataset_paths = {str(Path(key).parent) for key in keys}
        dataset_paths={parent for key in keys}
